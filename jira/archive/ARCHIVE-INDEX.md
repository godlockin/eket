...
```
jira/archive/
├── dropped/              (已废弃/被替代的票据, 48张)
│   ├── TASK-013.md
│   ├── TASK-014.md
│   └── ...
└── completed/            (已完成票据)
    ├── early/            (早期无创建时间, 84张)
    ├── 2026-04/          (2026-04完成, 95张)
    └── (每月新增...)
```
...
| 状态 | 说明 | 归档条件 |
|------|------|---------|
| `dropped` | 需求取消、方向变更、重复 | 人工标记废弃 |
| `superseded` | 被新任务取代 | 功能被其他ticket覆盖 |
| `closed` | 人工关闭（非完成） | 不再需要执行 |

### completed/ — 已完成票据按月归档

| 归档时间 | 条件 | 目的 |
|---------|------|------|
| 每月初 | 状态=done 且创建时间=上月 | 保持 tickets/ 聚焦当前工作 |
...
|------|------|------|
| `dropped/` | 48 | 已废弃/被替代 |
| `completed/early/` | 84 | 早期无创建时间 |
| `completed/2026-04/` | 95 | 2026-04 完成 |
| **总计** | **227** | 已归档票据 |

当前 `jira/tickets/`: 43 张（主要为 2026-05 + 其他状态）
