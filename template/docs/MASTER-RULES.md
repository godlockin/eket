...
### 9.2.1 上下文溢出防御（Context Overflow Prevention）

**EPIC/Milestone 完成流程的强制防御规则：**

#### 规则 1: 禁止大文件回读
```bash
# ❌ 禁止
Write(M1-completion-report.md, 200 行)
Read(M1-completion-report.md)  # 上下文翻倍！
...
**强制要求**：
- Write 后**禁止**自动 Read（除非用户明确要求）
- 文件 >100 行时，输出压缩到 1 行状态
...
# ✅ 正确
git log --oneline -3 --no-decorate短格式 `%h %s`

#### 规则 3: completion 输出压缩
```bash
# ❌ 禁止（50+ 行祝贺文案）
🎉 EPIC-009 M1 完成！
## 任务完成统计
...
# ✅ 正确（≤200 chars）
✅ EPIC-009 M1 完成。Report: jira/epics/EPIC-009/M1-report.md
```

**强制要求**：
- EPIC/Milestone 完成输出 ≤200 字符
- 禁止祝贺文案
- 仅输出：状态 + 文件路径

#### 规则 4: Proactive compact
```bash
# 大操作前强制检查
before_large_operation() {
  current_context=$(check_context_usage)
  if [ "$current_context" -ge 60 ]; then
    echo "⚠️ 上下文 ≥60%，执行 compact"
    /compact
  fi
}
```
