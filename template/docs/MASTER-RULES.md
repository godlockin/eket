...
### Rule 4：Master 负载分担 — 专属助理委托机制

当 Master 成为性能瓶颈时（并行 Slaver > 3 或消息队列积压 > 10），**必须**委托专属助理分担工作。
...
**外部 Supervisor**（独立于 Master 的守护进程）：

```bash
# scripts/supervisor.sh — 每 5 分钟 cron 执行
#!/bin/bash
...
**Master 恢复后**：优先处理 `recovery_queue.jsonl`，恢复中断的任务。
