...
- [ ] **时间盒参数已填入** — `{TIME_LIMIT}` = 估算工时 × 1.5（如 2h → 180，1h → 90）
- [ ] **文件读取限制已填入** — `{MAX_FILE_READS}` = 5（默认），特殊任务可调整为 3-10
- [ ] **防卡死规则已注入** — prompt 包含 `confluence/memory/guides/agent-prompt-template.md` 中"🚨 执行约束"段落
- [ ] **强制产出格式已明确** — prompt 包含结构化报告模板（Markdown 表格 + Top N 建议）
