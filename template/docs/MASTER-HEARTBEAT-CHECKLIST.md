...
### □ PR Review：4-Level Artifact Verification

PR review 时，按顺序检查以下 4 个层级：

| 级别 | 检查内容 | 常见反例 | 判断 |
|------|---------|---------|------|
| L1 存在性 | 声称新增/修改的文件在 diff 中可见 | PR 描述说"添加了 X"但 diff 里没有 | 缺 → reject |
| L2 实质性 | 实现是真实逻辑，非空壳 | `function foo() {}` / `return undefined` | 缺 → reject |
| L3 接线 | 新代码被正确 import/注册/export | 新建了文件但没有被任何地方引用 | 缺 → reject |
| L4 数据流 | 核心路径有非纯-mock 测试 | 所有测试都 mock 掉真实逻辑 | 不适用于纯文档 PR |

**与现有防幻觉规则的关系**：
- L2（实质性）= 对应"禁止伪造测试结果"（测试通过但实现为空）
