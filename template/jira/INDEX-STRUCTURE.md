...
**状态管理原则**：
1. **索引文件是唯一真相源**：Ticket 状态以 `jira/index/by-status.md` 和 `jira/INDEX.md` 为准
2. **先更新索引，后读取详情**：Slaver 领取任务时，先更新索引文件状态，再去 ticket 文件查看详情
3. ** ticket 文件是详情存储**：ticket 文件存储详细描述、验收标准、执行日志等，但状态字段以索引为准

**优势**：
- 单一状态源，避免多文件不一致
- Master 和 Slaver 只需读取索引文件即可获取全局状态
- 支持高效的批量状态查询和统计
