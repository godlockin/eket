...
EOF
    ...
    # 保存 instance ID 到项目级文件
    echo "$instance_id" > ".eket/.instance_id"
...
update_alive_instances() {
    ...
    # 读取现有列表或创建新列表
    if [ ! -f ".eket/alive_instances.json" ]; then
        echo '{"instances": []}' > .eket/alive_instances.json
    fi
    
    # 添加当前实例（使用 jq 如果可用，否则简单追加）
    if command -v jq &> /dev/null; then
        ...
        jq --arg id "$instance_id" \
           ...
           '.instances += [{"id": $id, "role": $role, "specialty": $specialty, "started_at": $ts, "heartbeat": $ts}]' \
           .eket/alive_instances.json > "$temp_file"
        mv "$temp_file" .eket/alive_instances.json
    else
        # 简单追加（不使用 jq）
        echo "  - $instance_id ($role)" >> .eket/alive_instances.txt
    fi
}
...
2. 启动 Master: \`/eket-start\`
3. Master 分析需求并创建任务
4. 启动 Slaver: \`/eket-start\`（在新会话中）
5. Slaver 领取任务并开发
...
main() {
    echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}              EKET 项目初始化 v2.0.0                            ${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
    echo ""
    
    # 检查是否已有实例 ID
    if [ -f ".eket/.instance_id" ]; then
        local existing_id=$(cat .eket/.instance_id)
        echo -e "${YELLOW}⚠ 检测到现有实例: $existing_id${NC}"
        echo -e "如果你想创建新实例，请删除 .eket/.instance_id 文件"
        echo ""
        echo -e "继续使用现有实例？(y/n)"
        read -r response
        if [ "$response" != "y" ]; then
        ...
        # 显示现有实例信息
        if [ -f ".eket/instances/$existing_id/identity.yml" ]; then
            echo -e "${CYAN}当前实例信息:${NC}"
            cat ".eket/instances/$existing_id/identity.yml"
        fi
        exit 0
    fi
