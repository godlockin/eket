...
/**
 * TASK-609: Create alert file when auto-compact fails after retry
 */
async function createCompactFailureAlert(sessionId: string, estimatedTokens: number): Promise<void> {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const alertPath = path.join(
    process.cwd(),
    '.eket',
    'inbox',
    `compact-failure-${timestamp}.md`
  );
...
## Recommended Actions
1. Run \`/compact\` manually in Claude session
2. Check Claude CLI connectivity: \`claude --version\`
3. Review logs for network/permission issues

## Context
- Threshold: 120k tokens
- Current estimate: ${estimatedTokens} tokens
- Task continued with risk of hitting 200k limit

---
*Auto-generated by TASK-609 enhanced auto-compact*
`;

  try {
    // Master suggestion: use recursive mkdir
    ...
    console.error('❌ Failed to create alert file:', error);
  }
}
...
export async function runClaude(options: ClaudeRunOptions): Promise<ClaudeRunResult> {
  ...
  // TASK-602/609: Check if context needs compacting with retry + alert
  const sessionId = options.sessionId || 'unknown';
  if (contextTracker.shouldCompact(sessionId)) {
    let compacted = await contextTracker.triggerCompact(sessionId);

    // TASK-609: Retry once if failed
    if (!compacted) {
      console.warn('⚠️  Auto-compact failed, retrying once...');
      await new Promise(resolve => setTimeout(resolve, 2000)); // 2s delay
      compacted = await contextTracker.triggerCompact(sessionId);
    }

    // TASK-609: Create alert file if still failed
    if (!compacted) {
      console.error('❌ Auto-compact failed after retry');
      await createCompactFailureAlert(sessionId, contextTracker.getSessionTokens(sessionId));
      console.warn('⚠️  Proceeding with task, manual /compact recommended');
    }
  }
