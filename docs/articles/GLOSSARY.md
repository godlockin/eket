...
| **Adapter** | A thin client that translates one LLM tool's native primitives (Skills, Subagents, Hooks) into EKET protocol operations. Five shipped: `CLAUDE.md` (full), `CURSOR.md` (full), `CODEX.md` (degraded), `COPILOT.md` (degraded), `AGENTS.md` (universal fallback). The adapter is replaceable; the protocol is durable. | `docs/articles/12-multi-tool-support/en/article.md:43,177` |
| **Adapter Contract** | The five testable requirements an adapter must satisfy to be a first-class protocol participant: identity file, ticket YAML frontmatter, branch + Conventional Commits, hook events to HTTP server, status reports on phase boundaries. Anything beyond these five is the adapter author's choice. | `docs/articles/12-multi-tool-support/en/article.md:241-294` |
| **Cross-Tool Event Bridge** | The HTTP Hook Server that exposes 28 lifecycle events (`PreToolUse`, `PostToolUse`, `TeammateIdle`, `TaskCompleted`, …) on a single surface so that *any* adapter — native or shim — can participate in protocol-level event handling. Synonym for "Hook Server" in protocol-level prose. | `node/src/hooks/http-hook-server.ts:14-19,144-171` |
