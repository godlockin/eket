...
pub struct NodeExecutionResult {
    ...
    pub is_gate: bool,
    /// For gate nodes: whether the condition passed (exit 0)
...
impl NodeExecutor {
    ...
    pub async fn execute(&self, node: &ReadyNode) -> NodeExecutionResult {
        // Handle gate nodes specially - execute condition instead of script
        if node.node_type == NodeType::Gate {
            return self.execute_gate(node).await;
        }
    ...
    /// Execute a gate node - runs condition command
    async fn execute_gate(&self, node: &ReadyNode) -> NodeExecutionResult {
        let start = std::time::Instant::now();
        let condition = node.condition.as_ref().unwrap_or(&node.script);

        debug!(
            "[NodeExecutor] Executing gate {} with condition: {}",
            node.id,
            sanitize_script(condition, 50)
        );

        let execute_future = self.run_command(condition);
        ...
        match timeout(timeout_duration, execute_future).await {
            Ok(result) => {
                ...
                match result {
                    Ok((exit_code, stdout, stderr)) => {
                        ...
                        NodeExecutionResult {
                            node_id: node.id.clone(),
                            success: true, // Gate always "succeeds" - it just determines the path
                    ...
                    Err(e) => {
                        error!("[NodeExecutor] Gate {} execution error: {}", node.id, e);
                        NodeExecutionResult {
                            node_id: node.id.clone(),
                            success: false,
                            exit_code: None,
                            stdout: String::new(),
                            stderr: String::new(),
                            error_msg: Some(format!("Gate execution error: {}", e)),
            ...
            Err(_) => {
                error!(
                    "[NodeExecutor] Gate {} timed out after {}s",
                ...
                NodeExecutionResult {
                    ...
                    stdout: String::new(),
                    stderr: String::new(),
                    error_msg: Some(format!("Gate timeout after {}s", node.timeout)),
                    duration_ms: start.elapsed().as_millis() as u64,
                    is_gate: true,
                    gate_passed: false,
                }
...
impl DryRunExecutor {
    ...
    pub async fn execute(node: &ReadyNode) -> NodeExecutionResult {
        ...
        let is_gate = node.node_type == NodeType::Gate;
        NodeExecutionResult {
            node_id: node.id.clone(),
            success: true,
            exit_code: Some(0),
            ...
            duration_ms: 100,
            is_gate,
            gate_passed: is_gate, // In dry-run, gates always pass
        }
    }
}
...
mod tests {
    ...
    fn make_gate_node(id: &str, condition: &str, timeout: u64) -> ReadyNode {
        ReadyNode {
            id: id.to_string(),
            script: String::new(),
            retry: 0,
    ...
    // TASK-641: Gate node execution tests
    #[tokio::test]
    async fn test_execute_gate_success() {
        let executor = NodeExecutor::with_defaults();
        let node = make_gate_node("gate-check", "exit 0", 10);
    ...
    #[tokio::test]
    async fn test_execute_gate_failure() {
        let executor = NodeExecutor::with_defaults();
        let node = make_gate_node("gate-check", "exit 1", 10);

        let result = executor.execute(&node).await;
        // Gate still "succeeds" (doesn't error), but condition failed
        assert!(result.success);
        assert!(result.is_gate);
        assert!(!result.gate_passed);
    }
    ...
    #[tokio::test]
    async fn test_execute_gate_with_actual_check() {
        let executor = NodeExecutor::with_defaults();
        // Test with actual condition - check if file exists
        let node = make_gate_node("gate-check", "test -f /etc/passwd", 10);

        let result = executor.execute(&node).await;
        assert!(result.success);
        assert!(result.is_gate);
        assert!(result.gate_passed); // /etc/passwd should exist on Unix
    }
}
